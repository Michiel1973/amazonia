...
local function cache_protocol_version(name)
...
local function get_protocol_version(name)
...
function chat3.colorize(name, colour, msg)
	local vers = get_protocol_version(name)
...
function chat3.send(name, msg, prefix, source)
	...
	for _, player in pairs(minetest.get_connected_players()) do
		...
		if (not vers or (vers and (vers >= 29 or (vers < 29 and name ~= rname))))
			...
			-- Send message
			local send = chat3.colorize(rname, colour, oclock.." ["..name.."]: "..msg)
			if prefix then
				send = prefix..send
			end
