...
local edge = tonumber(minetest.settings:get("world_edge")) or 30500
...
minetest.register_globalstep(function(dtime)   
	...
	for k, v in pairs(waiting_list) do
		if v.player and v.player:is_player() then
			...
			elseif not v.notified then
				v.notified = true
				minetest.chat_send_player(k, "You have reached the edge of the world, waiting to wrap you to other side...")
			end
		else
			v.obj:remove()
			waiting_list[k] = nil
	...
	for i, player in ipairs(players) do
		...
		if not waiting_list[name] then
			...
			-- Teleport the player
			if newpos then
				minetest.chat_send_player(name, "You are at the edge of the world, we will teleport you soon to the other side.")
