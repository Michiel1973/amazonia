...
minetest.register_craftitem("rover:rover", {
	description = "Rover",
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if pointed_thing.type == "node" and
			...
			under.y = under.y + 1.5
			local rover = minetest.add_entity(under, "rover:rover")
			if rover then
...
minetest.register_entity("rover:rover", rover)
...
function rover.on_punch(self, puncher, time_from_last_punch,
	...
	if not self.driver then
		...
		if not minetest.setting_getbool("creative_mode")
				or not inv:contains_item("main", "rover:rover") then
			local leftover = inv:add_item("main", "rover:rover")
...
function rover:on_step(dtime)
	...
	if nodedef_in.walkable then
		...
		if not touch_ground then
			...
			if node_under.name == "cratermg:dust" then
				minetest.set_node(under_pos, {name = "cratermg:dusttrack"})
			end
		end
	end
end
