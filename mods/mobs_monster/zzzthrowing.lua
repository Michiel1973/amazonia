--code from MCmobs v0.5 throwing by
--maikerumine
...
-- local c = mobs_monster.is_item_variable_overridden

minetest.register_node("mobs_monster:arrow_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
	...
	tiles = {"throwing_arrow.png^[transformFX", "throwing_arrow.png^[transformFX", "throwing_arrow_back.png", "throwing_arrow_front.png", "throwing_arrow.png", "throwing_arrow.png^[transformFX"},
...
local THROWING_ARROW_ENTITY={
	...
	textures = {"mobs_monster:arrow_box"},
	velocity = 10,
	lastpos={},
...
THROWING_ARROW_ENTITY.on_step = function(self, dtime)
	...
	if self.lastpos.x~=nil then
		if node.name ~= "air" then
			...
			minetest.add_item(self.lastpos, 'mobs_monster:arrow')
			self.object:remove()
		end
	end
...
minetest.register_entity("mobs_monster:arrow_entity", THROWING_ARROW_ENTITY)

local arrows = {
	{"mobs_monster:arrow", "mobs_monster:arrow_entity" },
}
...
-- if c("arrow") then
	-- minetest.register_craftitem("mobs_monster:arrow", {
		-- description = S("Arrow"),
...
-- if c("arrow") and c("flint") and c("feather") and c("stick") then
	-- minetest.register_craft({
		-- output = 'mobs_monster:arrow 4',
		-- recipe = {
			-- {mobs_monster.items.flint},
			-- {mobs_monster.items.stick},
			-- {mobs_monster.items.feather},
		-- }
	-- })
-- end

-- if c("bow") then
	-- minetest.register_tool("mobs_monster:bow_wood", {
		-- description = S("Bow"),
	...
	-- minetest.register_craft({
		-- output = 'mobs_monster:bow_wood',
		-- recipe = {
			-- {mobs_monster.items.string, mobs_monster.items.stick, ''},
			-- {mobs_monster.items.string, '', mobs_monster.items.stick},
			-- {mobs_monster.items.string, mobs_monster.items.stick, ''},
		-- }
	-- })
...
-- if c("egg") then
	...
	-- mobs:register_arrow("mobs_monster:egg_entity", {
		-- visual = "sprite",
		-- visual_size = {x=.5, y=.5},
		...
		-- hit_node = function(self, pos, node)
			...
			-- local mob = minetest.add_entity(pos, "mobs_monster:chicken")
	...
	-- minetest.register_craftitem("mobs_monster:egg", {
		-- description = S("Egg"),
...
-- mobs:register_arrow("mobs_monster:snowball_entity", {
	-- visual = "sprite",
	-- visual_size = {x=.5, y=.5},
	-- textures = {"throwing_snowball.png"},
	...
	-- hit_mob = function(self, mob)
		...
		-- local dmg = {}
		-- if mob:get_luaentity().name == "mobs_monster:blaze" then
			-- dmg = {fleshy = 3}
		-- end
		-- -- FIXME: No knockback
...
-- if c("snowball") then
	...
	-- -- Snowball
	-- minetest.register_craftitem("mobs_monster:snowball", {
		...
		-- _doc_items_usagehelp = how_to_throw,
		-- inventory_image = "throwing_snowball.png",
