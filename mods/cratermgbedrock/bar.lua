...
local function update_bar(pos)
    if minetest.env:get_node(pos).name:find("cratermgbedrock:bar") == nil then
        return
    end
    local sum = 0
    for i = 1, 4 do
    ...
    minetest.env:add_node(pos, {name = "cratermgbedrock:bar_"..sum})
end
...
for i = 1, 15 do
    ...
    minetest.register_node("cratermgbedrock:bar_"..i, {
        drawtype = "nodebox",
        tiles = {"bedrock_bar_white.png", "bedrock_bar_white.png", texture},
        paramtype = "light",
        groups = {unbreakable = 1},
        drop = "cratermgbedrock:bar",
        node_box = {
            type = "fixed",
            fixed = take
        },
...
minetest.register_node("cratermgbedrock:bar", {
...
minetest.register_craft({
	output = 'cratermgbedrock:bar 16',
	recipe = {
		{'cratermgbedrock:bedrock', '', 'cratermgbedrock:bedrock'},
        	{'cratermgbedrock:bedrock', '', 'cratermgbedrock:bedrock'},
        	{'cratermgbedrock:bedrock', '', 'cratermgbedrock:bedrock'}		
	}
})
